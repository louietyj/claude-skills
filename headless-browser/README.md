...
The shim screenshots after every page-changing command (`nav`, `click`, `fill`, ...) to `~/pinchtab-shots/NNNN-<cmd>.jpg` (`PINCHTAB_SHOT_DIR` overrides; not `/tmp`, which claude.ai won't show the user) and prints the path with its dimensions, saving the agent a separate screenshot call. It adds ~150ms per command; a bare pinchtab call is ~70ms. Changes a click makes synchronously, or within a couple of animation frames, are in the shot; content that waits on the network may not be yet. `PINCHTAB_AUTOSHOT=0` turns it off.
