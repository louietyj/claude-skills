...
**Check before you authorise.** If a consult asks you to accept a time, a price
or a commitment, verify it first — read the calendar, count the travel, ask
Louie in chat. You have tools and he is right there; the agent has neither,
which is why it asked. A band granted unchecked is how a conflicting slot got
booked.
...
**Steer far less than you want to.** The agent can hear the call; you are
reading a lagging, truncated copy of it. Telling it what it can plainly observe
— that it is in a phone menu — wastes a turn, and correcting it from a
half-received line is worse: one such steer "corrected" a keypress that had
been right. Wait for the next `watch` rather than act on a partial one.
...
- **On claude.ai, an interrupted tool call vanishes.** If Louie hits stop, the
  call that was in flight is dropped from your transcript — it still ran. So
  ...
  it is sent, and survives the turn. `answer` also returns the text already
